...
try:
    ...
    raise ImportError(f"❌ 错误: 无法导入 e3fp，请检查路径 {e3fp_lib_path} 是否存在 e3fp 文件夹。详细报错: {e}")
...
def worker_process_molecule(data_tuple):
    ...
    try:
        ...
        if not smiles or not coordinates_list: return None
        ...
        if num_atoms != len(coords_array): return None

        conf = Chem.Conformer(num_atoms)
        for i in range(num_atoms):
            conf.SetAtomPosition(i, Point3D(float(coords_array[i][0]), float(coords_array[i][1]), float(coords_array[i][2])))
        mol.AddConformer(conf)

        fprint_params = {'bits': FP_BITS, 'rdkit_invariants': True, 'level': FP_LEVEL, 'all_iters': True, 'exclude_floating': False}
...
def main():
    print(f"🚀 开始提取 PubChemQC 3D E3FP 特征...")

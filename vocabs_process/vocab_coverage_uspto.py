...
# ================= 绝对路径与配置 =================
BASE_DIR = os.path.expanduser("~/autodl-tmp/e3fp-uspto-50k/data")
...
def process_and_convert_parquet(parquet_path):
    ...
    # 检查是否已经转换过，避免重复工作
    if 'rect_smiles' not in df.columns or 'prod_smiles' not in df.columns:
        print(f"🔄 正在将 SELFIES 极速解码为 SMILES: {os.path.basename(parquet_path)}...")
        # 启用 pandas 的 apply 进行解码
        tqdm.pandas(desc="解码反应物")
        df['rect_smiles'] = df['rect_selfies'].progress_apply(decode_selfies)
        
        tqdm.pandas(desc="解码产物")
        df['prod_smiles'] = df['prod_selfies'].progress_apply(decode_selfies)
        
        # 存盘！后续就可以直接读带有 _smiles 的 parquet 了
    ...
    # 所以我们用 '.' 把两边连起来，当作一个超级分子复合体进行测试
    combined_smiles = df['rect_smiles'] + "." + df['prod_smiles']
...
def main():
    print("🚀 开始执行 USPTO-50k 数据集升维 (SELFIES -> SMILES) 及覆盖率测试...")
    ...
    for split_name, parquet_path in SPLITS.items():
        print("\n" + "="*60)
        print(f"🔬 正在处理下游任务: 【USPTO-50k {split_name}】")
        print("="*60)

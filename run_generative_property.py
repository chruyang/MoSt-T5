...
class GenerativeHybridDataset(Dataset):
    def __init__(self, json_path: str, lmdb_path: str,
        ...
        self.env = lmdb.open(lmdb_path, readonly=True, lock=False, readahead=False, meminit=False, subdir=os.path.isdir(lmdb_path))
        ...
        logger.info(f"✅ 索引建立完成，分子数: {len(self.cid_to_key):,}")
    ...
    def __getitem__(self, idx):
        for attempt in range(10):
            try:
                ...
                text_ids = self.text_tokenizer.tokenizer(prompt, truncation=True, max_length=self.max_source_length//2, return_tensors="pt")['input_ids'].squeeze(0)
                len_t = len(text_ids)
                ...
                target_ids = self.text_tokenizer.tokenizer(target_text, truncation=True, max_length=self.max_target_length, return_tensors="pt")['input_ids'].squeeze(0)
...
class GenerativeCollator:
    ...
    def __call__(self, batch):
        ...
        input_ids = pad_sequence([f['input_ids'] for f in valid_batch], batch_first=True, padding_value=self.text_pad_id)
        ...
        atom_to_motif_map = pad_sequence([f['atom_to_motif_map'] for f in valid_batch], batch_first=True, padding_value=-1)
        
        # 2. 只有原本不是 -1 的地方，才是真实的原子
        atom_attention_mask = (e3fp_ids[:, :, 0] != -1).long()
        
        # 3. 🚨 将所有传入 Embedding 的 e3fp 负数变为 0
        # 这是给 modeling.py 的 Embedding(padding_idx=0) 准备的“白名单”数据！
        safe_e3fp_ids = e3fp_ids.clone()
        safe_e3fp_ids[safe_e3fp_ids == -1] = 0

        # 4. 🚨 同样把 map 中越界的部分削掉，避免 clamp 导致第 0 个 token 爆炸
...
def main():
    ...
    train_dataset = GenerativeHybridDataset(os.path.join(args.data_dir, "train/3d_computed_properties_unit.json"), os.path.join(args.data_dir, "pubchemqc_final.lmdb"), text_tokenizer, motif_tokenizer, e3fp_tokenizer, max_source_length=768)
    eval_dataset = GenerativeHybridDataset(os.path.join(args.data_dir, "valid/3d_computed_properties_unit.json"), os.path.join(args.data_dir, "pubchemqc_final.lmdb"), text_tokenizer, motif_tokenizer, e3fp_tokenizer, max_source_length=768, is_eval=True)

    training_args = Seq2SeqTrainingArguments(
        output_dir="./checkpoints/gen_prop_SingleGPU_Stable",
        
        per_device_train_batch_size=4,     
        gradient_accumulation_steps=32,    # 1卡 * 4BS * 32Acc = 128 (保守起见再降一半等效 Batch)
        ...
        bf16=False, fp16=False,            # 🛡️ 坚持 FP32 防御
        ...
        dataloader_num_workers=2,          # 回归单卡保守 I/O
    ...
    logger.info("🚀 Starting 100% Modeling-Aligned Safe Fine-Tuning...")
    trainer.train()

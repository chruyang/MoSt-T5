...
状态：主干完成代码结构、文献先例和审稿逻辑复核。本文件细化文档 34 的问题 1，并把已接受的问题 2–4 正式并入后续总体计划。2026-08-05 的第二轮“优雅性”复核已将 EMA teacher 降为条件候选，将 matched ECFP/Morgan 和 shell-overlap 降为归因/数据诊断；第三轮复核进一步把“3D-MolT5 式双重均值”降为必要基线，并增加一个可被实验否决的低秩接口状态残差候选。它不表示已经修改训练代码或放行 P1。

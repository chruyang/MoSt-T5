...
fallback serializes RDKit's canonical isomeric SMILES with the same pinned
...
does not weaken molecular identity to admit an exceptional molecule.
...
SCHEMA_VERSION = "most-t5-next/fragsmiles-lossless-fallback/v2"
...
class LosslessFallbackSurface:
    ...
    def __post_init__(self) -> None:
        ...
        if decode_fallback_smiles(self.tokens) != self.canonical_isomeric_smiles:
...
def _project_with_source_indices(source_mol: Chem.Mol) -> Chem.Mol:
    ...
    projected = Chem.RemoveHs(indexed, sanitize=True)
...
def encode_lossless_fallback(source_mol: Chem.Mol) -> LosslessFallbackSurface:
    ...
    if replay_smiles != surface.canonical_isomeric_smiles:
        raise LosslessFallbackError("fallback strict isomeric round trip failed")
...
def decode_lossless_fallback_mol(tokens: Sequence[str]) -> Chem.Mol:
    ...
    canonical, _, _ = _canonical_cycle_member(_project_with_source_indices(mol))
    if canonical != smiles:

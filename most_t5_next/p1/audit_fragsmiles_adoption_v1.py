...
def _sequence_to_reduction_mapping(parsed_graph, reduction_graph, memberships):
    ...
    reduction_nodes = tuple(reduction_graph.nodes)
    if (
        len(parsed_nodes) != len(reduction_nodes)
        or parsed_graph.number_of_edges() != reduction_graph.number_of_edges()
    ):
        raise FragSmilesAuditError("serialized fragSMILES graph does not match reduction graph")

    # The former implementation enumerated *all* VF2 isomorphisms and only
    # then selected the lexicographically smallest canonical-atom membership
    # sequence.  Highly symmetric polymers and perfluoro chains can have an
    # exponential number of equivalent mappings, making a single valid record
    # take hours.  Search in the desired lexicographic order instead and stop
    # at the first complete mapping.  The result is identical to sorting every
    # full isomorphism, while the local labelled-neighbour signatures reject
    # impossible candidates before recursion.
    def node_key(graph, node):
        attrs = graph.nodes[node]
        return (attrs.get("smiles"), tuple(sorted(attrs.get("chirality", {}).items())))

    def edge_key(graph, left, right):
        attrs = graph.edges[left, right]
        return (attrs.get("aB"), attrs.get("stereo"))

    def unmapped_signature(graph, node, used, *, outgoing):
        neighbours = graph.successors(node) if outgoing else graph.predecessors(node)
        rows = []
        for neighbour in neighbours:
            if neighbour in used:
                continue
            if outgoing:
                edge = edge_key(graph, node, neighbour)
            else:
                edge = edge_key(graph, neighbour, node)
            rows.append((node_key(graph, neighbour), edge))
    ...
    ordered_candidates = {
        parsed: tuple(
            sorted(
                (
                    reduction
                    for reduction in reduction_nodes
                    if _node_match(
                        parsed_graph.nodes[parsed], reduction_graph.nodes[reduction]
                    )
                    and parsed_graph.in_degree(parsed)
                    == reduction_graph.in_degree(reduction)
                    and parsed_graph.out_degree(parsed)
                    == reduction_graph.out_degree(reduction)
                ),
                key=lambda reduction: tuple(memberships[reduction]),
            )
        )
    ...
    def extend(position):
        if position == len(parsed_nodes):
            return True
        parsed = parsed_nodes[position]
        assigned_parsed = set(mapping)
        for reduction in ordered_candidates[parsed]:
            if reduction in used_reduction:
                continue
            compatible = True
            for prior_parsed, prior_reduction in mapping.items():
                parsed_forward = parsed_graph.has_edge(parsed, prior_parsed)
                reduction_forward = reduction_graph.has_edge(reduction, prior_reduction)
                parsed_reverse = parsed_graph.has_edge(prior_parsed, parsed)
                reduction_reverse = reduction_graph.has_edge(prior_reduction, reduction)
                if parsed_forward != reduction_forward or parsed_reverse != reduction_reverse:
                    compatible = False
                    break
                if parsed_forward and not _edge_match(
                    parsed_graph.edges[parsed, prior_parsed],
                    reduction_graph.edges[reduction, prior_reduction],
                ):
                    compatible = False
                    break
                if parsed_reverse and not _edge_match(
                    parsed_graph.edges[prior_parsed, parsed],
                    reduction_graph.edges[prior_reduction, reduction],
                ):
                    compatible = False
                    break
            if not compatible:
                continue
            if unmapped_signature(
                parsed_graph, parsed, assigned_parsed, outgoing=True
            ) != unmapped_signature(
                reduction_graph, reduction, used_reduction, outgoing=True
            ):
                continue
            if unmapped_signature(
                parsed_graph, parsed, assigned_parsed, outgoing=False
            ) != unmapped_signature(
                reduction_graph, reduction, used_reduction, outgoing=False
            ):
                continue
            mapping[parsed] = reduction
            used_reduction.add(reduction)
            if extend(position + 1):
                return True
            used_reduction.remove(reduction)
            del mapping[parsed]
        return False

    if not extend(0):
        raise FragSmilesAuditError("serialized fragSMILES graph does not match reduction graph")
    return parsed_nodes, dict(mapping)

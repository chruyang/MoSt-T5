...
from most_t5_next.r1.auditor import audit_pcqm_geometry_production_release_v3 as audit


R1_ROOT = Path(__file__).resolve().parents[2]
AUDIT_CONTRACT = R1_ROOT / "contracts" / "p1_pcqm_geometry_independent_audit_contract_v3.json"
...
def fixture_runtime():
    return {
        "schema_version": audit.AUDIT_RUNTIME_SCHEMA,
        "python": {
        ...
        "libc": {"name": "glibc", "version": "2.31"},
        "numpy_version": "1.24.4",
        "python_lmdb_version": "1.7.5",
        "liblmdb_version": "0.9.33",
        "byteorder": "little",
    }
...
class IndependentProductionAuditTest(unittest.TestCase):
    def test_complete_streaming_closure_and_preregistered_sample_pass(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            report = audit.audit_release(
                ...
                runtime_probe=lambda _: fixture_runtime(),
            )
            ...
            self.assertEqual(rows[0]["auditor_script"], report["auditor_script"])
            self.assertEqual(
                rows[0]["audit_runtime_observation_sha256"],
                report["audit_runtime_observation_sha256"],
            )
            self.assertEqual(rows[0]["audit_runtime_observation"], fixture_runtime())
            self.assertEqual(
                rows[0]["production_contract_sha256"], audit.sha256_file(PRODUCTION_CONTRACT)
            )
            self.assertEqual(rows[0]["payload_contract_sha256"], audit.sha256_file(PAYLOAD_CONTRACT))
            self.assertEqual(sum(row["document_kind"] == "reject_semantic_review" for row in rows), 1)
            self.assertEqual(
                report[audit.REPORT_PAYLOAD_HASH_FIELD],
                audit.report_canonical_payload_sha256(report),
            )
            audit.validate_audit_report_document(report)
    ...
    def test_auditor_script_lock_rejects_mid_run_byte_drift(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            release, payloads = build_release(root)
            initial = audit.observe_auditor_script()
            changed = dict(initial)
            changed["sha256"] = "b" * 64
            with mock.patch.object(
                audit, "observe_auditor_script", side_effect=[initial, changed]
            ):
                with self.assertRaisesRegex(RuntimeError, "script bytes changed"):
                    audit.audit_release(
                        release, AUDIT_CONTRACT, PRODUCTION_CONTRACT, PAYLOAD_CONTRACT,
                        root / "audit-output",
                        lmdb_opener=lambda _: FakeEnvironment(payloads),
                        runtime_probe=lambda _: fixture_runtime(),
                    )
            self.assertFalse((root / "audit-output").exists())

    def test_runtime_observation_missing_field_is_rejected_without_version_equality(self):
        observed = fixture_runtime()
        del observed["liblmdb_version"]
        with self.assertRaisesRegex(RuntimeError, "fields are not closed"):
            audit.validate_audit_runtime_observation(observed)
        cpu = fixture_runtime()
        gpu = fixture_runtime()
        gpu["python"]["executable"] = "/different-host/python"
        gpu["platform"]["release"] = "different-kernel"
        gpu["numpy_version"] = "2.0.0"
        # Both observations are individually valid; v3 records the difference
        # instead of treating CPU/GPU environment equality as an audit gate.
        audit.validate_audit_runtime_observation(cpu)
        audit.validate_audit_runtime_observation(gpu)
        self.assertNotEqual(audit.sha256_json(cpu), audit.sha256_json(gpu))

    def test_report_canonical_payload_hash_detects_any_payload_tamper(self):
        report = {
            "schema_version": audit.AUDIT_REPORT_SCHEMA,
            "audit_status": "pass",
            "audit_class": "independent_full_envelope_and_deterministic_sampled_payload_v3",
            "release_id": "fixture",
            "release_manifest_sha256": HASH,
            "audit_contract_sha256": HASH,
            "production_contract_sha256": HASH,
            "payload_contract_sha256": HASH,
            "auditor_script": {
                "file_name": "auditor.py", "bytes": 123, "sha256": HASH,
            },
            "audit_runtime_observation": fixture_runtime(),
            "audit_runtime_observation_sha256": audit.sha256_json(fixture_runtime()),
            "counts": {},
            "passed_checks": [],
        ...
        report[audit.REPORT_PAYLOAD_HASH_FIELD] = audit.report_canonical_payload_sha256(report)
        audit.validate_audit_report_document(report)
        tampered = json.loads(json.dumps(report))
        tampered["counts"]["unexpected"] = 1
        with self.assertRaisesRegex(RuntimeError, "canonical payload hash mismatch"):
            audit.validate_audit_report_document(tampered)

    def test_report_write_is_strictly_exclusive_create(self):
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "independent_audit_report.json"

...
QM9_ROW_SCHEMA = "most-t5-r1/qm9-clean-split-member/v1"
QM9_SUMMARY_SCHEMA = "most-t5-r1/qm9-clean-split-summary/v1"
HIV_ROW_SCHEMA = "most-t5-r1/hiv-derived-split-member/v1"
HIV_SPLIT_SCHEMA = "most-t5-r1/hiv-murcko-derived-split-manifest/v1"
COLLECTION_SCHEMA = "most-t5-r1/identity-collection-manifest/v1"
IDENTITY_ROW_SCHEMA = "most-t5-r1/molecule-identity-row/v1"
SUMMARY_SCHEMA = "most-t5-r1/qm9-hiv-identity-collection-adapter-summary/v1"

QM9_PROTOCOL_ID = "qm9-3dmolt5-idgroup-110k10k-rest-s42-v1"
HIV_PROTOCOL_ID = "HIV-MoleculeNet/DeepChem-Murcko-8:1:1-derived-v1"
...
def write_collection(
    ...
    *,
    ...
    collection_id = "{}-{}-identity-v1".format(dataset_slug, split)
    rows = [
        {
...
def collect_qm9(
    ...
    summary_path: Path,
) -> Tuple[Dict[str, Dict[str, Tuple[str, str]]], Dict[str, int]]:
    ...
    row_counts = {split: 0 for split in PROTECTED_SPLITS}
    for row in iter_jsonl(split_manifest_path, "QM9 split manifest"):
        ...
        row_counts[split] += 1
        previous = members[split].setdefault(group_id, (connectivity, stereo))
        if previous != (connectivity, stereo):
            raise IdentityAdapterError(
                "one QM9 molecule group maps to conflicting identities"
            )
    for split in PROTECTED_SPLITS:
        ...
        if len(members[split]) != expected_groups.get(split):
            raise IdentityAdapterError(
                "QM9 {} molecule-group count differs from summary".format(split)
            )
    return members, row_counts
...
def collect_hiv(
    ...
    for row in iter_jsonl(member_manifest_path, "HIV member manifest"):
        ...
        if member_id in members[split]:
            raise IdentityAdapterError("duplicate HIV member_id in one split")
        members[split][member_id] = identities
    for split in PROTECTED_SPLITS:
        if len(members[split]) != expected.get(split):
            raise IdentityAdapterError(
...
def build_identity_collections(
    *,
    ...
    qm9_members, qm9_row_counts = collect_qm9(qm9_split_manifest, qm9_summary)
    hiv_members = collect_hiv(hiv_member_manifest, hiv_split_manifest)
    ...
    for split in PROTECTED_SPLITS:
        collections.append(
            write_collection(
                ...
                split=split,
                source_namespace="qm9_clean_group_id",

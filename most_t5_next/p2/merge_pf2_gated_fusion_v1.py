#!/usr/bin/env python3
"""Merge matched M0-G/M1-G manifests and apply the frozen F-Gate rules."""
...
REPORT_SCHEMA = "most-t5-p2/zero-init-gated-fusion-paired-decision/v1"
FINAL_UPDATE = 1000
...
class FGateMergeError(ValueError):
    """The two condition artifacts are not one matched F-Gate experiment."""
...
def _read_json(path: Path) -> dict[str, object]:
    try:
        ...
        raise FGateMergeError(f"cannot read JSON artifact: {path}") from exc
    if not isinstance(payload, dict):
        raise FGateMergeError("JSON artifact must be an object")
    return payload


def _manifest_path(path: Path) -> Path:
    path = Path(path)
    return path / F_GATE_MANIFEST_NAME if path.is_dir() else path


def _finite_float(mapping: Mapping[str, object], key: str) -> float:
    value = mapping.get(key)
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise FGateMergeError(f"{key} is not numeric")
    value = float(value)
    if not math.isfinite(value):
        raise FGateMergeError(f"{key} is not finite")
    return value
...
def _condition(
    ...
    if manifest.get("schema_version") != CONDITION_SCHEMA:
        raise FGateMergeError("condition manifest schema differs")
    if manifest.get("status") != "pass":
        raise FGateMergeError("condition training did not complete")
    if manifest.get("fusion_contract") != FUSION_CONTRACT:
        raise FGateMergeError("condition fusion contract differs")
    optimization = manifest.get("optimization")
    if not isinstance(optimization, Mapping) or optimization.get(
        "protocol_id"
    ) != F_GATE_PROTOCOL_ID:
        raise FGateMergeError("condition optimization protocol differs")
    rows = manifest.get("conditions")
    if not isinstance(rows, list) or len(rows) != 1 or not isinstance(rows[0], dict):
        raise FGateMergeError("condition manifest must contain exactly one cell")
    row = dict(rows[0])
    if row.get("condition") != expected:
        raise FGateMergeError(f"expected {expected} condition manifest")
    if row.get("optimizer_updates") != FINAL_UPDATE:
        raise FGateMergeError("condition did not complete the frozen update budget")
    trajectory = row.get("geometry_gate_trajectory")
    if not isinstance(trajectory, list) or len(trajectory) != 2:
        raise FGateMergeError("condition lacks the two-point gate trajectory")
    if [value.get("update") for value in trajectory] != [500, 1000]:
        raise FGateMergeError("condition gate trajectory updates differ")
    for value in trajectory:
        ...
        if not math.isclose(
            ...
            raise FGateMergeError("condition gate trajectory is inconsistent")
    final_gate = row.get("final_geometry_gate")
    if final_gate != trajectory[-1]:
        raise FGateMergeError("condition final gate differs from its trajectory")
    return row
...
def _final_evaluation(row: Mapping[str, object]) -> dict[str, object]:
    ...
    if not isinstance(evaluations, list):
        raise FGateMergeError("condition evaluations are absent")
    final = [value for value in evaluations if value.get("update") == FINAL_UPDATE]
    if len(final) != 1 or not isinstance(final[0], dict):
        raise FGateMergeError("condition lacks one final evaluation")
    return dict(final[0])


def merge_pf2_gated_fusion(
    *,
    m0_manifest: Mapping[str, object],
    m1_manifest: Mapping[str, object],
) -> dict[str, object]:
    """Validate one matched pair and return the preregistered route."""
    ...
    for key in (
        ...
        if m0_manifest.get(key) != m1_manifest.get(key):
            raise FGateMergeError(f"matched condition contract differs at {key}")
    ...
    if not isinstance(m0_execution, Mapping) or not isinstance(m1_execution, Mapping):
        raise FGateMergeError("condition execution contract is absent")
    for key in (
        "forward_seed",
        ...
        if m0_execution.get(key) != m1_execution.get(key):
            raise FGateMergeError(f"matched execution differs at {key}")
    ...
    for key in (
        ...
        if m0.get(key) != m1.get(key):
            raise FGateMergeError(f"paired training exposure differs at {key}")
    ...
    for key in (
        ...
        if m0_eval.get(key) != m1_eval.get(key):
            raise FGateMergeError(f"paired final evaluation differs at {key}")
    ...
    if any(
        ...
        raise FGateMergeError("M0 geometry gate changed despite an inactive branch")
    final_sensitivity = m1.get("final_e3fp_shuffle_diagnostic")
    if not isinstance(final_sensitivity, Mapping):
        raise FGateMergeError("M1 lacks final geometry sensitivity")
    if final_sensitivity.get("update") != FINAL_UPDATE:
        raise FGateMergeError("M1 sensitivity is not from the final update")
    if m0.get("final_e3fp_shuffle_diagnostic") is not None:
        raise FGateMergeError("M0 unexpectedly reports a geometry diagnostic")
    ...
    sensitivity_gate = final_delta >= SENSITIVITY_ABSOLUTE_FLOOR
    non_degradation = nll_gate and accuracy_gate
    if non_degradation and sensitivity_gate:
        decision = "retain_f_gate_and_proceed_to_3d_sensitive_probe"
    elif non_degradation:
        decision = "enter_pf2b_t3mi"
    else:
        decision = "stop_formal_training_and_revisit_geometry_interface"
    ...
    return {
        ...
        "frozen_practical_gates": {
            ...
            "initial_aligned_shuffled_delta_by_construction": 0.0,
        },
        "metrics": {
            "m0_final_nll": m0_nll,
            "m1_final_nll": m1_nll,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.output.exists():
        raise FGateMergeError("output must be a new path")

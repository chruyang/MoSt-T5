...
class CompiledFragSmilesRecord:
    ...
    def __post_init__(self) -> None:
        ...
        if self.mode == MODE_TO_ID["compact"]:
            ...
            if fragment_count or self.endpoints or self.molecule_carrier != token_count - 1:
...
def compile_sidecar_record(
    *,
    ...
    if len(input_ids) != len(sidecar.model_tokens):
        raise FragSmilesTrainingCacheError("token IDs do not cover the model surface")
    ...
    return CompiledFragSmilesRecord(
        ...
        token_to_fragment=tuple(int(value) for value in sidecar.token_to_fragment),
...
def collate_cached_fragsmiles(
    ...
    """Dynamically pad one batch; random task corruption happens before this call."""
    ...
    for batch_index, row in enumerate(records):
        token_count, atom_count = len(row.input_ids), len(row.e3fp)
        input_ids[batch_index, :token_count] = torch.as_tensor(np.asarray(row.input_ids), dtype=torch.long)
        attention_mask[batch_index, :token_count] = True
        e3fp[batch_index, :atom_count] = torch.as_tensor(np.asarray(row.e3fp), dtype=torch.long)
        atom_mask[batch_index, :atom_count] = True
        atom_to_fragment[batch_index, :atom_count] = torch.as_tensor(np.asarray(row.atom_to_fragment), dtype=torch.long)
        atom_carrier[batch_index, :atom_count] = torch.as_tensor(np.asarray(row.atom_carriers), dtype=torch.long)
    return {
        "input_ids": input_ids,
